...
在 GitHub Copilot Chat 模型选择器中直接使用 AIXRouter 支持的模型，无需离开 Copilot Agent 模式。

喜欢 AIXRouter 的统一中转能力，但不想放弃 GitHub Copilot 的 Agent 模式、工具调用、Instructions、MCP 和成熟的交互体验？本扩展将 OpenAI 兼容模型接入 Copilot Chat 模型选择器，支持视觉输入、思考模式和自带 API Key。

## 为什么选这个扩展？

- 不是替换 Copilot，而是增强 Copilot。没有新的侧边栏，没有新的聊天界面，只是在模型选择器中多出 AIXRouter 模型。
...
- AIXRouter 模型列表默认从 `/models` 动态读取，也可以用设置固定展示指定模型。
...
2. 运行命令 `AIXRouter: Set Base URL`，输入你的 OpenAI 兼容 Base URL。
3. 运行命令 `AIXRouter: Set API Key`，粘贴你的 API Key。
4. 打开 Copilot Chat，点击模型选择器。
5. 选择 AIXRouter 提供的模型，开始使用 Agent 模式。
...
| 设置项 | 默认值 | 说明 |
| --- | --- | --- |
| `aixrouter.baseUrl` | 空 | OpenAI 兼容 API Base URL，首次安装后由用户输入 |
| `aixrouter.models` | `[]` | 固定模型列表。留空时从 `/models` 动态读取 |
